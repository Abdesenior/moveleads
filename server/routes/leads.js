...
router.get('/', auth, async (req, res) => {
  try {
    ...
    if (req.user.role !== 'admin') {
      ...
      //
      // Phase 6 — rejected-only routing: in 'rejected_only' mode the
      // available branch ALSO requires the lead to pass the mover-visibility
      // filter (hides REJECTED_FAKE, admin-overridden-rejected, and shadow-
      ...
      // Phase 6.2 — count how many leads were hidden from this mover by the
      // visibility filter. Only runs when a non-off mode is active. One small
      // countDocuments per feed pull; failure is swallowed so feed never breaks.
      //
      // Note: REJECTED_FAKE leads have status !== 'Available'/'READY_FOR_DISTRIBUTION',
      // so they're already excluded by the existing status filter and don't
      // appear in this count. We only count the *new* filter wins (admin override
      // = rejected, or shadowTier = rejected) — the actual signal we want to
      // measure during the first production window.
      if (routingMode() !== 'off') {
        Lead.countDocuments({
          status: { $in: ['Available', 'READY_FOR_DISTRIBUTION'] },
          moveDate: { $gte: new Date() },
          $and: [
            { $or: [{ sourceCompany: { $exists: false } }, { sourceCompany: req.user.id }] },
            { $or: [{ 'adminTierOverride.tier': 'rejected' }, { shadowTier: 'rejected' }] },
          ],
        }).then(n => { if (n > 0) recordFeedHidden(n); })
          .catch(err => console.warn('[leadVisibility] feed_hidden count failed:', err.message));
      }
