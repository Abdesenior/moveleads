...
router.get('/', auth, async (req, res) => {
  try {
    ...
    // If user is not admin, only show Available leads. Purchased leads are
    // intentionally EXCLUDED from this feed (purchase-flow cleanup Phase A,
    // 2026-05-26): they live exclusively in /dashboard/my-leads
    // (server: GET /api/purchases). Pre-Phase-A the query also returned
    // `{ 'buyers.company': req.user.id }` so the mover's purchased leads
    // re-appeared in the marketplace on every refresh — which conflicted
    // with the operator's UX intent (purchased leads should disappear
    // from the marketplace permanently).
    //
    // The client still does an optimistic `setLeads(prev => prev.filter(...))`
    // on successful purchase for instant UI responsiveness; this server
    // change makes the persistence truth match the optimistic update.
    if (req.user.role !== 'admin') {
      // A mover sees ONLY:
      // 1. Public marketplace leads (no sourceCompany) with a future move date.
      // 2. Leads generated by their own private widget.
      //
      ...
      const availableBranch = {
        ...
        // Belt-and-suspenders: explicitly exclude leads where this mover
        // is already a buyer. The lifecycle filter above (status in
        // Available/READY_FOR_DISTRIBUTION) already excludes most
        // purchased leads (they get status='Purchased' on buy-now), but
        // legacy multi-buyer leads (claim flow with maxBuyers > 1) can
        // remain Available after one mover has bought a slot. Guarding
        // here keeps the contract clean regardless of which purchase
        // path produced the buyers entry.
        'buyers.company': { $ne: req.user.id },
        ...moverVisibilityFilter(),                                // quality
      };
      query = availableBranch;
