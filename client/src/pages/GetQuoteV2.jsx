...
const FAQ_ITEMS = [
  ...
  { q: 'What if something goes wrong with my move?', a: 'We have a built-in Resolution Center. Submit a complaint privately — the mover is notified and given the chance to resolve the issue before it affects their public reputation.' },
];
...
/* ── Count-up hook ── */
function useCountUp(target, { duration = 1800, decimals = 0, threshold = 0.5 } = {}) {
  const ref = useRef(null);
      ...
      <div className="gqv2-est-row">
        ...
        <div className="gqv2-est-sizes">
          {Object.keys(BASES).map(s => (
            <button
              key={s}
              className={`gqv2-est-size${size === s ? ' gqv2-est-size--on' : ''}`}
              onClick={() => setSize(s)}
            >{s}</button>
          ))}
        </div>
...
export default function GetQuoteV2() {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setActVisible(false);
      setTimeout(() => { setActIdx(i => (i + 1) % ACTIVITIES.length); setActVisible(true); }, 350);
    }, 4000);
  ...
  return (
    <div className="gqv2-root">
      ...
      <nav className="gqv2-nav">
        ...
        <a href="tel:+13072044792" className="gqv2-nav-phone">
          <span>📞</span> +1 (307) 204-4792
        </a>
      </nav>
        ...
        <div className="gqv2-hero-inner">
          ...
          <div className="gqv2-hero-left">
            ...
            <h1 className="gqv2-hero-h1">
              Get Free Moving<br />Quotes in <span>60 Seconds</span>
            </h1>
            <p className="gqv2-hero-sub">
              Compare quotes from verified, licensed movers in your area. Free, no obligation, no spam calls — ever.
            </p>
            <div className="gqv2-hero-trust">
              {[
                'All movers are phone-verified and licensed',
                'Your info is never sold to third parties',
                'Only 1–3 matched movers contact you',
                'Free resolution center if anything goes wrong',
              ].map(t => (
                <div key={t} className="gqv2-trust-row">
                  <div className="gqv2-trust-check">✓</div>{t}
                </div>
              ))}
            </div>
      ...
      <section className="gqv2-how">
        <Anim>
          ...
          <h2 className="gqv2-section-h2">Three steps to your perfect mover</h2>
          <p className="gqv2-section-sub">No browsing endless listings. No callbacks from strangers. Just verified movers for your route.</p>
        </Anim>
        <div className="gqv2-steps">
          {[
            { n: '1', title: 'Tell us about your move',       desc: 'Home size, origin, destination, and move date. Takes under 60 seconds to complete.', connector: true },
            { n: '2', title: 'We match you with movers',      desc: 'Our system finds verified, licensed movers who service your exact route and filters out brokers.', connector: true },
            { n: '3', title: 'Movers contact you directly',   desc: 'Expect a call or text within 15 minutes. You compare offers and choose the best one.', connector: false },
          ].map(({ n, title, desc, connector }, i) => (
            <Anim key={n} delay={i * 120} className="gqv2-step-anim">
              <div className="gqv2-step-card">
                ...
                {connector && <div className="gqv2-step-connector" />}
              </div>
            </Anim>
          ))}
        </div>
        ...
        <div className="gqv2-why-inner">
          <Anim className="gqv2-why-photo-col">
            <img
              ...
              loading="lazy"
              onLoad={() => setMoversLoaded(true)}
              style={{ opacity: moversLoaded ? 1 : 0, transition: 'opacity 0.5s' }}
            />
          </Anim>
      ...
      <section className="gqv2-estimator-section">
        <Anim>
          <CostEstimator onGetQuote={scrollToHero} />
        </Anim>
      </section>
